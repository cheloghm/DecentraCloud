// src/components/Dashboard/FileViewDownload.js
...
const FileViewDownload = () => {
  ...
  const handleView = async () => {
    ...
    try {
      const response = await axios.get(`/api/storage/view/${user.id}/${filename}`, {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await axios.get(`/api/storage/download/${user.id}/${filename}`, {
        responseType: 'blob',
        headers: { Authorization: `Bearer ${user.token}` },
      });
